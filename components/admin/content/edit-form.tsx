...
import { updateClothes } from "@/lib/actions";
...
export default function EditForm({
    ...
    const [state, formAction] = useActionState(
        async (prevState: unknown, formData: FormData) =>
            updateClothes(prevState, formData, imageUrl ?? "", clothes.id),
        null
    );
    const checkedAmenities = clothes.ClothesAmenities.map((item) => item.amenitiesId);
            ...
            <div className="grid md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-8 bg-black p-6">
                    ...
                    <textarea
                        ...
                        defaultValue={clothes.description}
                        className="w-full mb-4 bg-transparent border border-gray-300 px-4 py-3 rounded-sm text-white"
                    />
                    <div aria-live="polite" aria-atomic="true">
                    ...
                    {amenities.map((item) => (
                        <div className="flex items-center mb-4" key={item.id}>
                            <label className="flex items-center gap-2 text-white">
                                <input type="checkbox" name="amenities" defaultValue={item.id} defaultChecked={checkedAmenities.includes(item.id)} />
                                {item.name}
                            </label>
                        </div>
                    ))}
